...
title: Filament loaded status per tool
...
# K-032 — Filament loaded status per tool

## Context

Filament loaded status per tool. Parser work to extract AMS data from Bambu MQTT + Snapmaker filament_feed/filament_detect objects.

See `.fleet/research-notes.md`'s K-055 section for the full AMS/filament
protocol reference (MQTT wire format, Moonraker endpoints, data model
