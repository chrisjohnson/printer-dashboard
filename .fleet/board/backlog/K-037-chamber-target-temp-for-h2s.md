...
title: Chamber target temp for H2S
...
# K-037 — Chamber target temp for H2S

## Context

Chamber target temp for H2S. Single MQTT command (set_ctt). Extends Printer interface with SetChamberTemp(). H2S-only feature.

See `.fleet/research-notes.md`'s K-054 section for the full printer-controls
protocol reference (command shapes, capability matrix) before starting.
