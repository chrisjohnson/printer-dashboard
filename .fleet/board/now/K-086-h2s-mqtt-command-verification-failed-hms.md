...
## Plan
1. [ ] Researcher: check `internal/server` logs / recent MQTT command history
...
2. [ ] Researcher: read `internal/printers/bambu/commands.go` and
...
3. [ ] Researcher: check whether this HMS is one-shot/transient or
...
4. [ ] Report findings, then hand to Implementer with a concrete fix once
   root cause is known (do not implement speculatively).
